...
async def get_aey_overview(days: int = Query(30, ge=1, le=365)):
    ...
    rows = await cached_execute(f"efficiency:aey:{days}", query)
...
async def get_cognitive_load(days: int = Query(30, ge=1, le=365)):
    ...
    rows = await cached_execute(f"efficiency:cognitive_load:{days}", query)
    return rows[0] if rows else {
...
async def get_feedback_latency(days: int = Query(30, ge=1, le=365)):
    ...
    rows = await cached_execute(f"efficiency:feedback_latency:{days}", query)
...
async def get_harness_convergence(days: int = Query(30, ge=1, le=365)):
    ...
    rows = await cached_execute(f"efficiency:harness_convergence:{days}", query)
...
async def get_rework_ratio(days: int = Query(30, ge=1, le=365)):
    ...
    rows = await cached_execute(f"efficiency:rework_ratio:{days}", query)
    return rows[0] if rows else {
        "avg_rework_ratio": 0,
