...
async def get_server_summary(
    ...
    rows = await cached_execute(f"mcp_server_summary:{server}:{days}", query)
    return {"servers": rows}
...
async def get_server_detail(
    ...
    rows = await cached_execute(f"mcp_server_detail:{server}:{days}", query)
...
async def get_tool_stats(
    ...
    rows = await cached_execute(f"mcp_tool_stats:{server}:{days}", query)
    return {"tools": rows}
...
async def get_tool_latency_timeline(
    ...
    rows = await cached_execute(f"mcp_tool_timeline:{server}:{days}", query)
...
async def get_http_outbound_summary(
    ...
    rows = await cached_execute(f"mcp_http_summary:{server}:{days}", query)
    return {"http": rows}
...
async def get_http_outbound_detail(
    ...
    rows = await cached_execute(f"mcp_http_detail:{server}:{limit}:{days}", query)
    return {"calls": rows}
...
async def get_tool_audit(
    ...
    rows = await cached_execute(f"mcp_audit:{server}:{limit}:{days}", query)
...
async def get_error_events(
    ...
    rows = await cached_execute(f"mcp_errors:{server}:{limit}:{days}", query)
    return {"errors": rows}
...
async def get_server_logs(
    ...
    rows = await cached_execute(f"mcp_logs:{server}:{limit}:{days}", query)
    return {"logs": rows}
