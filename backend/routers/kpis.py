...
# ── Bounded TTL cache for expensive queries ──
...
def _execute_sync(cache_key: str, query: str) -> list:
    ...
    logger.info("KPI query %s: %.0fms (%d rows)", cache_key, elapsed, len(rows))
    _cache[cache_key] = rows
    return rows


async def _cached_execute(cache_key: str, query: str) -> list:
    """Execute query off the event loop via thread pool."""
...
async def refresh_mat_view():
    """Refresh the KPI materialized view with latest data."""
    t0 = time.monotonic()
    query = kpi_service.build_refresh_mat_view()
    await asyncio.to_thread(require_pg_executor().execute, query, 60000)
    elapsed = (time.monotonic() - t0) * 1000
    _cache.clear()
    return {"status": "refreshed", "elapsed_ms": round(elapsed)}
...
async def get_cost_overview(days: int = Query(30, ge=1, le=365)):
    ...
    rows = await _cached_execute(f"cost_overview:{days}", query)
...
async def get_cost_trend(days: float = Query(30, ge=0.01, le=365)):
    ...
    rows = await _cached_execute(f"cost_trend:{days}", query)
...
async def get_cost_sessions(
    ...
    rows = await _cached_execute(f"cost_sessions:{days}:{limit}", query)
...
async def get_cost_models(days: int = Query(30, ge=1, le=365)):
    ...
    rows = await _cached_execute(f"cost_models:{days}", query)
...
async def get_cost_waste(days: int = Query(30, ge=1, le=365)):
    ...
    rows = await _cached_execute(f"cost_waste:{days}", query)
...
async def get_effectiveness_overview(days: int = Query(30, ge=1, le=365)):
    ...
    rows = await _cached_execute(f"effectiveness_overview:{days}", query)
    return rows[0] if rows else {
...
async def get_effectiveness_retries(days: int = Query(30, ge=1, le=365)):
    ...
    rows = await _cached_execute(f"effectiveness_retries:{days}", query)
...
async def get_effectiveness_orphans(days: int = Query(30, ge=1, le=365)):
    ...
    rows = await _cached_execute(f"effectiveness_orphans:{days}", query)
...
async def get_effectiveness_recovery(days: int = Query(30, ge=1, le=365)):
    ...
    rows = await _cached_execute(f"effectiveness_recovery:{days}", query)
...
async def get_effectiveness_complexity(days: int = Query(30, ge=1, le=365)):
    ...
    rows = await _cached_execute(f"effectiveness_complexity:{days}", query)
...
async def get_flow_summary(days: int = Query(30, ge=1, le=365)):
    ...
    rows = await _cached_execute(f"flow_summary:{days}", query)
...
async def get_efficiency_matrix(days: int = Query(30, ge=1, le=365)):
    ...
    rows = await _cached_execute(f"efficiency_matrix:{days}", query)
...
async def get_efficiency_rightsizing(days: int = Query(30, ge=1, le=365)):
    ...
    rows = await _cached_execute(f"efficiency_rightsizing:{days}", query)
...
async def get_efficiency_rightsizing_details(
    ...
    rows = await _cached_execute(f"efficiency_rightsizing_details:{days}:{model}:{complexity}:{limit}", query)
...
async def get_efficiency_recommendations(days: int = Query(30, ge=1, le=365)):
    ...
    rows = await _cached_execute(f"efficiency_recommendations:{days}", query)
...
async def get_efficiency_savings(days: int = Query(30, ge=1, le=365)):
    ...
    rows = await _cached_execute(f"efficiency_savings:{days}", query)
...
async def get_kpi_badges(days: int = Query(30, ge=1, le=365)):
    ...
    rows = await _cached_execute(f"badges:{days}", query)
    return rows[0] if rows else {
